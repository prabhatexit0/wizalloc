...
export function inferColumnTypes(
  ...
  return headers.map((rawName, colIdx) => {
    ...
    if (nonEmpty.length === 0) {
      return {
        name,
        type: { VarChar: 32 } as { VarChar: number },
        nullable: true,
      };
    }
    ...
    const rounded =
      maxLen <= 32 ? 32 : maxLen <= 64 ? 64 : maxLen <= 128 ? 128 : 255;
...
export function estimateByteSize(
  ...
  if (typeof colType === 'object') {
    if ('VarChar' in colType)
      return 2 + new TextEncoder().encode(String(value)).length;
    if ('Blob' in colType) return 2 + String(value).length / 2;
  }
  switch (colType) {
    case 'Int32':
    case 'UInt32':
